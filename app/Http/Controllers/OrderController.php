...
class OrderController extends Controller
{
    ...
    {
        $cipher = "AES-128-ECB";
        $key = hex2bin(trim($this->merchantKey)); // âœ… convert hex â†’ binary
        $encrypted = openssl_encrypt($plainText, $cipher, $key, OPENSSL_RAW_DATA);
        return strtoupper(bin2hex($encrypted));
    }
    
    private function decrypt($encryptedHex)
    {
        $cipher = "AES-128-ECB";
       $key = hex2bin(trim($this->merchantKey)); // âœ… convert hex â†’ binary
        $binary = hex2bin($encryptedHex);
        return openssl_decrypt($binary, $cipher, $key, OPENSSL_RAW_DATA);
    }
...
//           Log::info("=====paymentCallback:", request()->all());
//         // Unique hit ID for tracing
//         $hitId = now()->format('YmdHis') . '-' . rand(1000,9999);
//         Log::info("===== Callback Hit Start | ID: {$hitId} =====");

//         // Capture encrypted response
//         $encrypted = $request->get('meRes') ?? null;
//         Log::info("Encrypted Data | ID: {$hitId}: " . ($encrypted ?? 'empty'));

//         if (!$encrypted) {
//             Log::error("No encrypted data received | ID: {$hitId}");
//             return response('SUCCESS', 200)->header('Content-Type', 'text/plain');
//         }

//         // Decrypt response (make sure decrypt() is implemented)
//         $decrypted = $this->decrypt($encrypted);
//         // echo $decrypted;die;
//         Log::info("Decrypted Data | ID: {$hitId}: " . $decrypted);

//         // Split fields
//         $fields   = explode('|', $decrypted);
//         $order_id = $fields[1] ?? null;
//         $status   = $fields[4] ?? null;

//         Log::info("Callback Response for Order {$order_id} | Status: {$status} | ID: {$hitId}");

//         // Check DB for pending order
//         $data = DB::table('temp_order_hd')
//                 ->where('order_id', $order_id)
//                 ->where('tran_date', date('Y-m-d'))
//                 ->where('status_trans','pending')
//                 ->first();
...
//                     'reponce_data' => json_encode($fields)
//                 ]);

//             // Redirect user to receipt page
//             return redirect()->route('receipt.show', ['orderId' => $order_id]);

//         } else {
//             // Update order to failure
//             DB::table('temp_order_hd')
//                 ->where('order_id', $order_id)
//                 ->where('tran_date', date('Y-m-d'))
//                 ->where('status_trans','pending')
//                 ->update(['status_trans'=>'failure']);

//             // Redirect to payment failed page
//             return redirect()->route('payment.failed', ['orderId' => $order_id]);
//         }

//         // Always return plain SUCCESS to bank
//         return response('SUCCESS', 200)->header('Content-Type', 'text/plain');
//     }
...
//     {
//         $hd_data = DB::table('temp_order_hd')
...
//         if (!$hd_data) {
//             return redirect()->route('items.index')->with('error', 'Receipt not found.');
//         }
...
//         $dt_data = DB::table('temp_order_dt')
//             ->select('temp_order_dt.*', 'item_master.item_desc', 'item_master.item_hdesc', 'item_master.item_gst as igst', 'item_master.item_rate')
//             ->join('item_master', 'temp_order_dt.item_code', '=', 'item_master.item_code')
//             ->join('temp_order_hd', 'temp_order_dt.tran_no', '=', 'temp_order_hd.tran_no')
//             ->where('temp_order_hd.tran_no', $trans_no)
//             ->where('temp_order_dt.tran_date', date('Y-m-d'))
//             ->where('temp_order_hd.tran_date', date('Y-m-d'))
//             ->where('temp_order_hd.status_trans', 'success')
//             ->where('temp_order_dt.tran_no', $trans_no)
//             ->get();

//         // Payment Mode
//         switch ($hd_data->payment_mode) {
//             case 'O': $paymentMode = 'Online'; break;
//             case 'C': $paymentMode = 'Cash'; break;
//             case 'U': $paymentMode = 'Counter UPI'; break;
//             case 'Z': $paymentMode = 'Zomato'; break;
//             case 'S': $paymentMode = 'Swiggy'; break;
//             default:  $paymentMode = 'Unknown'; break;
//         }

//         return view('items.bill_old', compact('dt_data', 'hd_data', 'rest_data', 'paymentMode'));
//     }
    

//     // 🔹 Show Failure Page
//     public function showFailure($orderId)
//     {
//         return view('items.payment_failed', compact('orderId'));
//     }



//     /** ðŸ”Ž Transaction Status Enquiry **/
//     public function checkStatus()
//     {
//         $orderId = 'ORD1757483347';
//         $message = $this->merchantId."|".$orderId."||||||||||NA";
//                         // HDFC000000000086|||65437829217889||||||||||NA
//         $encryptedMsg = $this->encrypt($message);
        
//         // echo $encryptedMsg;die;
...
//         $response = Http::withoutVerifying()->post("https://testupi.mindgate.in:8443/hupi/transactionStatusQuery", $payload);
        
//          $rawResponse = $response->body();
        
//         // return response()->json(["status" => $rawResponse]);

//         // $data = $response->json();
//         $decrypted = $this->decrypt($rawResponse);

//         return response()->json(["status" => $decrypted]);
//     }

//     /** ðŸ’¸ Refund API **/
//     public function refund($orderId, $rrn, $upiTxnId, $amount)
//     {
//         $refundOrderId = "REF" . time();
//         $message = $this->merchantId . "|" . $refundOrderId . "|" . $orderId . "|" . $upiTxnId . "|" . $rrn . "|Refund|" . $amount . "|INR|P2M|PAY||||||||||NA";

//         $encryptedMsg = $this->encrypt($message);

//         $payload = [
//             "request-Msg" => $encryptedMsg,
//             "pgMerchantId" => $this->merchantId,
//         ];

//         $response = Http::withoutVerifying()->post($this->uatUrl . "refundReqSvc", $payload);
//         $data = $response->json();
//         $decrypted = $this->decrypt($data["responseMsg"] ?? "");

//         return response()->json(["refund_status" => $decrypted]);
//     }
    ...
    public function dashboard(){
        ...
        foreach ($paymodeSales as $row) {
         ...
         // Ã¢Å“â€¦ 1. TodayÃ¢â‚¬â„¢s total sales
    $todaySales = DB::table('order_hd')
    ->where('tran_date', date('Y-m-d'))
    ->sum('paid_amt');

// Ã¢Å“â€¦ 2. Monthly overall sales (last 6 months)
        ...
        public function updateStatus(Request $request)
    {
        ...
        if($minutes!='none'){
            if ($itemStatus === 'A') {
                ...
                $futureTime = now('Asia/Kolkata')->addMinutes($minutes)->format('H:i:s'); // Ã°Å¸â€˜Ë† Only time string
...
//    echo"<pre>";print_r($request->all());die;
    ...
    $response = Http::withHeaders([
        ...
        'x-merchantid' => '60473',
        'x-customerid' => '355',
    ])->post('https://smartgateway.hdfcbank.com/orders/', [
        'order_id' => $orderId,
    ]);
    ...
    if(!empty($data) && $data->paid_amt == $amount && $status == 'CHARGED'){
        ...
        if($hd_data->payment_mode=='O'){
         ...
         return view('items.bill_single', compact('dt_data', 'hd_data', 'rest_data','paymentMode','invoiceNo'));
    }else{
