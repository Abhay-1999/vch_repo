    ...
    public function index()
{
    ...
    // Average Sales Mix
    $avgSalesMix = 100 / max(count($tempRows), 1);

    // Average Contribution Margin
    $avgMargin = collect($tempRows)
        ->avg('contribution_margin');
    ...
    foreach ($tempRows as $row) {
        ...
        // Sales Mix %
        $salesMix = $totalUnitsSold > 0
            ? ($row['units_sold'] / $totalUnitsSold) * 100
            : 0;

        // Margin Mix %
        $marginMix = $grandMargin > 0
            ? ($row['total_margin'] / $grandMargin) * 100
            : 0;
