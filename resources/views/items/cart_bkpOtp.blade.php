      ...
      @if(session('cart') && count(session('cart')) > 0)
        ...
        <form id="orderForm" action="{{ route('payment.dummy') }}" method="POST">
          ...
          <div class="mb-2 text-center ">
              <label for="mobile" class="form-label" style="font-size: 0.875rem;">Mobile Number:</label>
              <div>
                  <input type="text" id="mobile" name="mobile" 
                      style="width: 150px; padding: 4px 8px; font-size: 0.8rem;" 
                      placeholder="Enter number" required>
                  <button type="button" id="sendOtpBtn" 
                      style="padding: 4px 8px; font-size: 0.8rem;" 
                      class="btn btn-secondary btn-sm">Send OTP</button>
              </div>
          </div>

          <!-- OTP Verification -->
          <div class="mb-2" id="otpSection" style="display: none;">
              <label for="otpInput" class="form-label" style="font-size: 0.875rem;">Enter OTP:</label>
              <div>
                  <input type="text" id="otpInput" 
                      style="width: 100px; padding: 4px 8px; font-size: 0.8rem;" 
                      placeholder="OTP">
                  <button type="button" id="verifyOtpBtn" 
                      style="padding: 4px 8px; font-size: 0.8rem;" 
                      class="btn btn-success btn-sm">Verify</button>
              </div>
              <p id="otpMessage" class="text-success mt-1" style="display: none; font-size: 0.8rem;">Mobile verified successfully!</p>
          </div>

          <!-- Payment Options -->
          <div class="mb-2 " style="font-size: 0.85rem;">
              <label><input type="radio" name="paymode_mode" value="O" checked disabled> Pay Online</label>
              <!-- <label class="ms-3"><input type="radio" name="paymode_mode" value="C" disabled> Pay Over the Counter</label> -->
          </div>

          <!-- Confirm Order Checkbox -->
          <div class="mb-2" style="font-size: 0.85rem;">
              <input type="checkbox" name="confirm_order" value="Y"> Confirm your order
          </div>

          <!-- Submit Button -->
          <p style="font-size: 0.75rem; color:white;">By clicking Pay Now, you agree to our <a href="{{ route('terms') }}" target="_blank">Terms & Conditions</a>.</p>

          <button type="submit" class="btn btn-primary btn-sm mt-2" id="payNowBtn" disabled style="font-size: 0.8rem; padding: 4px 12px;">
              Pay Now
          </button>
      </form>
